...
const getOnlineTeams = async (req, res) => {
    ...
    try {
        ...
        const onlineTeams = database.collection('teams');
        const result = await onlineTeams.find({}).toArray();
        res.send(result);
    } catch (error) {
        console.error('MongoDB connection FAIL');
        res.send('MongoDB connection FAIL');
    } finally {
        await client.close();
    }
};

const getOfflineTeams = async (req, res) => {
    const client = new MongoClient(process.env.MONGO_URI, {
        useNewUrlParser: true,
        useUnifiedTopology: true,
    });
    ...
    try {
        await client.connect();
        const database = client.db('test');
        const offlineTeams = database.collection('offteaminfos');
        const result = await offlineTeams.find({}).toArray();
        res.send(result);
    } catch (error) {
        console.error('MongoDB connection FAIL');
        res.send('MongoDB connection FAIL');
...
const getCentreInfo = async (req, res) => {
    ...
    try {
        ...
        const result = await centreInfo.find({}).toArray();
        res.send(result);
    } catch (error) {
        console.error('MongoDB connection FAIL');
        res.send('MongoDB connection FAIL');
...
module.exports = { getOnlineTeams, getOfflineTeams, getCentreInfo, createAdmitCard, getAdmitCard };